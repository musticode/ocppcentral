...
import { getActivePricingForChargePoint, getPricePerKwh } from './pricingService.js'
...
async function getTariffForTransaction(transaction) {
  ...
  let priceInfo = await tariffService.getPriceForConnector(transaction.chargePointId, transaction.connectorId, dateTime)
  ...
  const chargePoint = await chargePointService.getChargePointByIdentifier(transaction.chargePointId)
  if (chargePoint && chargePoint._id) {
    priceInfo = await tariffService.getPriceForConnector(chargePoint._id.toString(), transaction.connectorId, dateTime)
    if (priceInfo && priceInfo.tariff) return priceInfo
  }
  return null
}
...
export const createConsumptionFromTransaction = async transactionId => {
  ...
  try {
    ...
    if (priceInfo && priceInfo.tariff) {
      ...
      pricing = await getActivePricingForChargePoint(transaction.chargePointId, transaction.startedAt)

      if (pricing) {
        pricePerKwh = pricing.getPriceForDateTime(transaction.startedAt)
        connectionFee = pricing.connectionFee || 0
        currency = pricing.currency || 'USD'
        console.log(`Using pricing for transaction ${transactionId}: ${pricing.name}`)
      }
    }
  } catch (error) {
    console.warn(`Could not get pricing/tariff for transaction ${transactionId}:`, error.message)
  }
  ...
  if (existingConsumption) {
    throw new Error(`Consumption record already exists for transaction ${transactionId}`)
  }
  ...
  // Calculate duration
  const duration =
    transaction.stoppedAt && transaction.startedAt ? (transaction.stoppedAt - transaction.startedAt) / 1000 : null
...
export const createConsumption = async consumptionData => {
  ...
  if (consumptionData.transactionId) {
    ...
    if (transaction) {
      consumptionData.chargePointId = consumptionData.chargePointId || transaction.chargePointId
      consumptionData.connectorId = consumptionData.connectorId || transaction.connectorId
      consumptionData.idTag = consumptionData.idTag || transaction.idTag
      consumptionData.meterStart = consumptionData.meterStart || transaction.meterStart
      consumptionData.transactionStartTime = consumptionData.transactionStartTime || transaction.startedAt
        ...
        try {
          ...
          if (priceInfo && priceInfo.tariff) {
            ...
            if (pricing) {
              ...
              consumptionData.pricePerKwh = pricing.getPriceForDateTime(transaction.startedAt || new Date())
...
export const fetchUserConsumptionByDate = async (userId, startDate, endDate) => {
  if (!userId) return []
...
export const fetchAllConsumptionByUserId = async userId => {
  if (!userId) return []
  const consumption = await Consumption.find({ userId: userId }).populate('pricingId')
  return consumption
}
